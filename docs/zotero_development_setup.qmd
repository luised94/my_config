...
### BetterBibtex
Citation Key Formula: auth.fold + year + title.lower.skipwords().select(1,1)
 > Using auth and year is very simple and how I am accostumed to refer to citations. However, when there is a duplicate, it just appends a letter and is not very memorable and relative to addition data. Adding the first letter of the title does not add too much text but adds a bit of context. Does add overhead to the bib file if exporting.
...
Fields to export:
 > abstract,url, urldate, annotation, langid, shorttitle, shortjournal, eprint, eprinttype, eprintclass, issn, isbn, pubstate, pagetotal, entrysubtype, version, pmcid, origdate, year, namea, nameatype, editora, editoratype, editortype, type, edition, issue, journalsubtitle, volumes, howpublished, eissn, collaboration, bookauthor, translator, eventtitle, institution, organization, series, abstract, file, annote, note
 > Create a smaller bib export for other tools that mostly just need citation key. (zotero/console_js/bbt_export.js)

Leave others as default.
Refresh keys if necessary.

Reference: 
- https://claude.ai/chat/44a9fa22-1018-4515-909e-9e2ab3a87105
- https://www.perplexity.ai/search/can-i-export-tags-in-zotero-by-4kXk9SszTam_FlHQponwow
### Better Notes
...
### OCRmyPdf on ~400 files
Wanted to rename and move 400 files to make sure they were in Dropbox. Accidentally clicked on OCRmyPdf...
Tried to see if I could find a way to do it from the console but I failed.
While pdftoppm is running, you can close and wait for pdftoppm to finish. That should stop everything. Restart computer just to be safe. Double check with any PDFs to check for corruption. Remove the leftover jpgs.
